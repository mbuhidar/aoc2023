...
def reversed_layers(readings: list[list[int]]) -> list[list[int]]:
    ...
    return reversed_layers(readings)


def compute(s: str) -> int:
    lines = s.splitlines()

    sum_of_prior_values = 0

    for line in lines:
        readings = [[int(reading) for reading in line.split(' ')]]
        tree = reversed_layers(readings)
        tree = tree[::-1]
        appended_lines = []
        appended_line = []
        ...
        sum_of_prior_values = sum_of_prior_values + appended_line[-1]

    return sum_of_prior_values
